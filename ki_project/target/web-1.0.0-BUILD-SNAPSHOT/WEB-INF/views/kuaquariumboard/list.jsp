...
<body>
   ...
   <script>
      ...
      $(document).ready(function() {
               ...
               $("#btn-register").on("click", function() {
                  self.location = "/kuaquariumboard/register${pageMaker.makeSearch(cri.page)}";
               });
  ...
  <div id="page-wrapper">
      ...
      <div class="container-fluid">
        <div class="col-lg-12">
              <h1 class="page-header">KudanAquarium</h1>
               </div>
               <div class="row">
         ...
         <c:forEach items="${list}" var="kuAquariumBoardVO">
            <tbody>
               <tr align="center">
                  <td>${kuAquariumBoardVO.id_num}</td>
                  <td><a
                     href='/kuaquariumboard/readPage${pageMaker.makeQuery(pageMaker.cri.page) }&id_num=${kuAquariumBoardVO.id_num}'>
                        ${kuAquariumBoardVO.title}</a></td>
                  <td>${kuAquariumBoardVO.latitude}</td>
                  <td>${kuAquariumBoardVO.longitude}</td>
                  <td>${kuAquariumBoardVO.karmarker}</td>
                  <td>${kuAquariumBoardVO.regdate}</td>
               </tr>
            </tbody>
               ...
               <th colspan="3">
                  <div class="ui pagination menu">
                     <c:if test="${pageMaker.prev}">
                        <a class="icon item"
                           href="/kuaquariumboard/list${pageMaker.makeSearch(startPage - 1)}">&laquo;</a>
                     </c:if>
                     ...
                     <c:forEach begin="${pageMaker.startPage }"
                        ...
                        <div
                           ...
                           <a class="icon item"
                              href="/kuaquariumboard/list${pageMaker.makeSearch(idx)}">${idx}</a>
                        </div>
                     </c:forEach>

                     <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
                        <a class="icon item"
                           href="/kuaquariumboard/list${pageMaker.makeSearch(pageMaker.endPage +1)}">&raquo;</a>
                     </c:if>
                  </div>
               </th>
            </tr>
