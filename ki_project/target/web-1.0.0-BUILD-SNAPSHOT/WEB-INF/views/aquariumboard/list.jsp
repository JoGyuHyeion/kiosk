...
<body>
   ...
   <script>
      ...
      $(document).ready(function() {
               ...
               $("#btn-register").on("click", function() {
                  self.location = "./register${pageMaker.makeSearch(cri.page)}";
               });
  ...
  <div id="page-wrapper">
      ...
      <div class="container-fluid">
        <div class="col-lg-12">
              <h1 class="page-header">Aquarium</h1>
               </div>
               <div class="row">
                                   ...
                                   <table width="100%"  class="table table-striped table-bordered table-hover">
                                      <thead>
                                        <tr>
                                             ...
                                             <th><center><b>Dataset.xml</b></center></th>
                                             <th><center><b>Regdate</b></center></th>
                                         </tr>
                                   </thead>

         <c:forEach items="${list}" var="AquariumBoardVO">
            <tbody>
               <tr align="center">
                  <td>${AquariumBoardVO.id_num}</td>
                  <td><a
                     href='/aquariumboard/readPage${pageMaker.makeQuery(pageMaker.cri.page) }&id_num=${AquariumBoardVO.id_num}'>
                        ${AquariumBoardVO.title}</a></td>
                  <td>${AquariumBoardVO.latitude}</td>
                  <td>${AquariumBoardVO.longitude}</td>
                  <td>${AquariumBoardVO.xml}</td>
                  <td>${AquariumBoardVO.regdate}</td>
               </tr>
            </tbody>
               ...
               <th colspan="3">
                  <div class="ui pagination menu">
                     <c:if test="${pageMaker.prev}">
                        <a class="icon item"
                           href="/aquariumboard/list${pageMaker.makeSearch(startPage - 1)}">&laquo;</a>
                     </c:if>
                     ...
                     <c:forEach begin="${pageMaker.startPage }"
                        ...
                        <div
                           ...
                           <a class="icon item"
                              href="/aquariumboard/list${pageMaker.makeSearch(idx)}">${idx}</a>
                        </div>
                     </c:forEach>

                     <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
                        <a class="icon item"
                           href="/aquariumboard/list${pageMaker.makeSearch(pageMaker.endPage +1)}">&raquo;</a>
                     </c:if>
                  </div>
               </th>
            </tr>
