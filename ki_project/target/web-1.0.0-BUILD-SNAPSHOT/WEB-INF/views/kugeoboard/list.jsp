...
<body>
   ...
   <script>
      ...
      $(document).ready(function() {
               ...
               $("#btn-register").on("click", function() {
                  self.location = "/kugeoboard/register${pageMaker.makeSearch(cri.page)}";
               });
  ...
  <div id="page-wrapper">
      ...
      <div class="container-fluid">
        <div class="col-lg-12">
              <h1 class="page-header">KudanGeo</h1>
               </div>
               <div class="row">
         ...
         <c:forEach items="${list}" var="KuGeoBoardVO">
            <tbody>
               <tr align="center">
                  <td>${KuGeoBoardVO.id_num}</td>
                  <td><a
                     href='/kugeoboard/readPage${pageMaker.makeQuery(pageMaker.cri.page) }&id_num=${KuGeoBoardVO.id_num}'>
                        ${KuGeoBoardVO.title}</a></td>
                  <td>${KuGeoBoardVO.latitude}</td>
                  <td>${KuGeoBoardVO.longitude}</td>
                  <td>${KuGeoBoardVO.karmarker}</td>
                  <td>${KuGeoBoardVO.regdate}</td>
               </tr>
            </tbody>
               ...
               <th colspan="3">
                  <div class="ui pagination menu">
                     <c:if test="${pageMaker.prev}">
                        <a class="icon item"
                           href="/kugeoboard/list${pageMaker.makeSearch(startPage - 1)}">&laquo;</a>
                     </c:if>
                     ...
                     <c:forEach begin="${pageMaker.startPage }"
                        ...
                        <div
                           ...
                           <a class="icon item"
                              href="/kugeoboard/list${pageMaker.makeSearch(idx)}">${idx}</a>
                        </div>
                     </c:forEach>

                     <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
                        <a class="icon item"
                           href="/kugeoboard/list${pageMaker.makeSearch(pageMaker.endPage +1)}">&raquo;</a>
                     </c:if>
                  </div>
               </th>
            </tr>
